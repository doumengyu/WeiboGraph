...
class Client:
    ...
    def get_friendship(self,sourceid,targetid):
        try:
            return self.client.friendships.show.get(source_id=sourceid,target_id=targetid)
        except Exception ,e:
			print e
			current_index = int(random.random()*100 % self.length)
			self.client=self.access_client(current_index)
			return self.get_friendship(sourceid,targetid)
